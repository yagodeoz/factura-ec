...
@ManagedBean(name = "beanMantendorComprobantesError")
@ViewScoped
public class BeanMantendorComprobantesError extends
	...
	public BeanMantendorComprobantesError() {
	...
	private void consultaDatos() {
		...
		*/
		...
		addMensaje("Consulta realizada con exito, total de comprobantes: Error: " + lListaComprobantesError.size());
	...
	public void eliminar(ActionEvent accion) {
		try {
			ComprobanteElectronico lComprobante = (ComprobanteElectronico) accion.getComponent().getAttributes()
					.get("COMPROBANTE");
			
			lServicioComprobanteEliminar.eliminarRegistro(lComprobante.getId());
			
			consultaDatos();
			
			addMensaje("Comprobante eliminado fisicamente de la base de datos");
